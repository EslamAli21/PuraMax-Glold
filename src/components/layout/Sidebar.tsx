...
const DEFAULT_NAV_GROUPS: NavGroup[] = [
    ...
    },
    {
      ...
      items: [
        { key: "nav.dustCollection", path: "/dust-collection", icon: Wind,    roles: ["Owner","Production Manager","Scale Operator"] },
        { key: "nav.refineryReturn",  path: "/refinery-return", icon: Repeat2, roles: ["Owner","Production Manager"] },
      ],
    },
    {
      id: "g-tracking",
...
function NavItemRow({
  ...
  const isActive = location === itemBasePath || (location.startsWith(itemBasePath + "/") && !hasChildren);
  ...
  const displayLabel = customLabels[item.key] || t(item.key);
  ...
  return (
    <div>
      ...
      {showChildren && isOpenState && isExpanded && (
