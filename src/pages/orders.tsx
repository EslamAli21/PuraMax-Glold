// ============================================================
// صفحة الطلبات — إدارة وعرض وتعديل طلبات العملاء
// ============================================================
...
// ── دالة تحديد أصناف CSS لشارة حالة الطلب بناءً على قيمة الحالة ──
// تُعيد سلسلة أصناف Tailwind مناسبة للوضع الفاتح والداكن معاً
const getStatusBadgeVariant = (status: OrderStatus) => {
  switch (status) {
    case "pending":       return "bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-400 border-amber-200 dark:border-amber-800";    // معلق
    case "approved":      return "bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-400 border-green-200 dark:border-green-800";    // معتمد
    case "in-production": return "bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-400 border-blue-200 dark:border-blue-800";         // في الإنتاج
    case "on-hold":       return "bg-orange-100 text-orange-800 dark:bg-orange-900/50 dark:text-orange-400 border-orange-200 dark:border-orange-800"; // متوقف
    case "completed":     return "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800"; // مكتمل
    case "cancelled":     return "bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-400 border-red-200 dark:border-red-800";               // ملغى
    default: return "";
  }
};

// ── مخطط التحقق من صحة نموذج الطلب الجديد باستخدام Zod ──
// يضمن إدخال جميع الحقول الإلزامية وصحة أنواع البيانات
const newOrderSchema = z.object({
  clientId:          z.string().min(1),                         // معرّف العميل — إلزامي
  modelId:           z.string().optional().or(z.literal("")),   // معرّف الموديل — اختياري
  itemName:          z.string().min(1),                         // اسم القطعة — إلزامي
  sizes:             z.string().min(1),                         // المقاسات — إلزامي
  qty:               z.coerce.number().min(1),                  // الكمية — على الأقل 1
  totalWeightGrams:  z.coerce.number().min(1),                  // الوزن الكلي (جرام) — على الأقل 1
  stampId:           z.string().min(1),                         // معرّف الختم — إلزامي
  deliveryDate:      z.string().min(1),                         // تاريخ التسليم — إلزامي
  notes:             z.string(),                                // ملاحظات — اختيارية
  isNewModel:        z.boolean().default(false),                // هل هو موديل جديد؟ — افتراضي: لا
});
...
export default function OrdersPage() {
  ...
  // Read URL ?status= param and jump to that tab on mount / navigation
  useEffect(() => {
    const VALID: OrderStatus[] = ["pending","approved","in-production","on-hold","completed","cancelled"];
    const read = () => {
      const s = new URLSearchParams(window.location.search).get("status");
      if (s && VALID.includes(s as OrderStatus)) setActiveTab(s as OrderStatus);
    };
    read();
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {/* ── عمود صورة الموديل المأخوذة من قسم الموديلات ── */}
              ...
              <TableBody>
                {orders.filter(o => o.status === activeTab).map(order => (
                  <TableRow key={order.id}>
                    ...
                    {/* ── خلية صورة الموديل: يتم جلب الصورة من قائمة الموديلات عبر modelId ── */}
                    <TableCell>
                      {(() => {
                        const mdl = order.modelId ? models.find(m => m.id === order.modelId) : null;
                        return mdl?.image ? (
                          /* صورة الموديل — تتكبّر عند المرور وتفتح نافذة ضوئية عند النقر */
                          <ZoomableImage src={mdl.image} alt={mdl.name} className="w-10 h-10 object-cover rounded-md border border-border/50" />
                        ) : (
