...
function OrdersTabContent() {
  ...
  const [dateFrom] = useState("04/01/2026");
  const [dateTo]   = useState("04/30/2026");
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border bg-card p-4 space-y-3 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          {/* Date range */}
          <div className="flex items-center gap-1.5 px-3 py-2 rounded-md border bg-background text-sm text-muted-foreground cursor-default select-none">
            <Calendar className="w-3.5 h-3.5 shrink-0" />
            <span className="font-mono text-xs">{dateFrom} – {dateTo}</span>
          </div>
              ...
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.map((order, rowIdx) => {
                  ...
                  return (
                    <tr
                      key={order.id}
                      className={`border-b last:border-b-0 border-blue-100 dark:border-blue-900/50 hover:bg-blue-100/60 dark:hover:bg-blue-900/25 transition-colors ${isEven ? "bg-white dark:bg-card" : "bg-blue-50/60 dark:bg-blue-950/20"}`}
                    >
                      {/* 1. Order Date & Time */}
      ...
      {/* ── MODELS TABLE ──────────────────────────────────────────────────── */}
      {(() => {
        // Collect all order+model rows from filtered orders
          ...
          <div className="rounded-xl border overflow-hidden shadow-sm">
            ...
            <div className="flex items-center justify-between px-4 py-2.5 bg-emerald-600 dark:bg-emerald-800">
              ...
              <span className="text-emerald-100 text-xs">
                {isRTL ? `${modelRows.length} موديل` : `${modelRows.length} model${modelRows.length !== 1 ? "s" : ""}`}
              </span>
            </div>
              ...
              <table className="w-full text-xs border-collapse min-w-[2200px]">
                ...
                <tbody>
                  {modelRows.map(({ order, model, stamp, lastMov, totalLossG, totalLossPct, finalGoldW, pureGoldPct, materialW, totalFinalW, progress, currentSection, lastWorker, lastMachine, qrCode, isEven, modelIdx }) => (
                    <tr
                      key={`${order.id}-model`}
                      className={`border-b last:border-b-0 border-emerald-100 dark:border-emerald-900/50 hover:bg-emerald-100/60 dark:hover:bg-emerald-900/20 transition-colors ${isEven ? "bg-white dark:bg-card" : "bg-emerald-50/50 dark:bg-emerald-950/15"}`}
                    >
                      {/* 1. Model NO */}
                      ...
                      <td className="px-3 py-2.5 border-e border-border/40 min-w-[140px]">
                        <span className="block font-bold font-mono text-primary text-[11px]">{model?.code ?? "—"}</span>
                        <span className="block text-muted-foreground text-[10px] truncate max-w-[130px]">{model?.name ?? (isRTL ? "موديل مخصص" : "Custom")}</span>
                      </td>
      ...
      {/* ── PARTS TABLE ──────────────────────────────────────────────────────── */}
      {(() => {
        // Collect all part rows from every filtered order whose model has parts
        const partRows: {
      ...
      {/* ── ROAD MAP TABLE ───────────────────────────────────────────────── */}
      {(() => {
        // Build one row per filtered order that has a model
        type RoadRow = {
          order: typeof filteredOrders[0];
...
function DepartmentsTab() {
    ...
    const deptStats = sections.map((s: any) => {
      const sMovs         = movements.filter((m: any) => m.toSectionId === s.id);
    ...
    // Active departments = sections that have at least one movement
    const activeDepts           = deptStats.filter((d: any) => d.activeOrders > 0 || d.completedOrders > 0 || movements.some((m: any) => m.toSectionId === d.section.id)).length || sections.length;
    // Total orders that touched at least one department (unique set)
    const allWorkedOrderIds     = [...new Set(movements.map((m: any) => m.orderId))];
    const totalOrdersWorked     = allWorkedOrderIds.length;
    const totalHoursWorked      = deptStats.reduce((a: number, d: any) => a + d.hoursWorked, 0);
    // Qty processed: use unique orders that went through movements, avoid double-counting
...
function WorkersTab() {
  ...
  const workerStats = workers.map((w: any, idx: number) => {
    ...
    const wMovs     = movements.filter((m: any) => m.workerId === w.id);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border bg-card p-4 space-y-3 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <div className="flex items-center gap-1.5 px-3 py-2 rounded-md border bg-background text-muted-foreground cursor-default select-none">
            <Calendar className="w-3.5 h-3.5 shrink-0" />
            <span className="font-mono text-xs">{dateFrom} – {dateTo}</span>
          </div>
      ...
      <div className="flex gap-4 items-start">
        ...
        {selectedWorker && (
          <div className="w-[340px] shrink-0 space-y-4 sticky top-4">
            ...
            {/* ── 5. Hours Worked vs Output chart ─────────────────────────── */}
            <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
              <div className="px-4 py-2.5 border-b border-border/50">
                <p className="font-bold text-sm">{L("Workers: Hours Worked vs Output","العمال: ساعات العمل مقابل الإنتاج")}</p>
                <p className="text-[10px] text-muted-foreground mt-0.5">{L("Qty Processed (pcs)","الكمية المعالَجة (قطعة)")}</p>
              </div>
              <div className="p-3">
                <div className="h-[185px] w-full" dir="ltr">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={chartData} margin={{ left: 0, right: 14, top: 5, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border))" />
                      <XAxis dataKey="name" stroke="hsl(var(--muted-foreground))" tickLine={false} axisLine={false} tick={{ fontSize: 9 }} />
                      <YAxis yAxisId="left" stroke="hsl(var(--muted-foreground))" tickLine={false} axisLine={false} tick={{ fontSize: 9 }} width={28} />
                      <YAxis yAxisId="right" orientation="right" stroke="hsl(var(--muted-foreground))" tickLine={false} axisLine={false} tick={{ fontSize: 9 }} width={28} />
                      <Tooltip
                      ...
                      <Bar yAxisId="left" dataKey="hours" fill="hsl(var(--chart-2))" radius={[3,3,0,0]} barSize={16}
...
function ComparisonsTab() {
  ...
  const compareData  = months.map(month => ({ month, thisYear: parseFloat((Math.random()*3+1).toFixed(2)), lastYear: parseFloat((Math.random()*3+1.2).toFixed(2)) }));
  const sectionCompare = sections.slice(0,5).map(s => ({ name: s.name, current: parseFloat((Math.random()*4+0.5).toFixed(2)), previous: parseFloat((Math.random()*4+0.8).toFixed(2)) }));
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader><CardTitle>Loss Rate — This Year vs Last Year</CardTitle><CardDescription>Monthly comparison of average loss percentage</CardDescription></CardHeader>
        <CardContent className="h-[300px]">
          <div className="w-full h-full" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={compareData} margin={{ left: 10, right: 10, top: 5, bottom: 5 }}>
                ...
                <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", borderColor: "hsl(var(--border))", borderRadius: "8px" }} />
                <Legend />
                <Line type="monotone" dataKey="thisYear" stroke="hsl(var(--chart-1))" strokeWidth={2} dot={{ r: 4 }} name="This Year" />
                <Line type="monotone" dataKey="lastYear" stroke="hsl(var(--chart-2))" strokeWidth={2} strokeDasharray="5 5" dot={{ r: 4 }} name="Last Year" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader><CardTitle>Section Comparison — Current vs Previous Period</CardTitle></CardHeader>
        <CardContent className="h-[280px]">
          <div className="w-full h-full" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sectionCompare} margin={{ left: 10, right: 10, top: 5, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border))" />
                <XAxis dataKey="name" stroke="hsl(var(--muted-foreground))" tickLine={false} axisLine={false} tick={{ fontSize: 11 }} />
                <YAxis stroke="hsl(var(--muted-foreground))" tickLine={false} axisLine={false} />
                <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", borderColor: "hsl(var(--border))", borderRadius: "8px" }} />
                <Legend />
                <Bar dataKey="current"  fill="hsl(var(--chart-1))" radius={[4,4,0,0]} barSize={20} name="Current Period" />
                <Bar dataKey="previous" fill="hsl(var(--chart-3))" radius={[4,4,0,0]} barSize={20} name="Previous Period" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
