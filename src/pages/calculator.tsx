...
// ── جدول نقاء كل عيار ذهب (نسبة الذهب الصافي في السبيكة) ──
// المفتاح: عيار الذهب — القيمة: نسبة الذهب الخالص (0-1)
const KARAT_PURITY: Record<string, number> = {
  "24K": 1.0,    // ذهب خالص 100%
  "22K": 0.9167, // 91.67% ذهب
  "21K": 0.875,  // 87.5% ذهب — الأكثر شيوعاً في الخليج
  "18K": 0.75,   // 75% ذهب
  "14K": 0.5833, // 58.33% ذهب
  "10K": 0.4167, // 41.67% ذهب
  "9K":  0.375,  // 37.5% ذهب
};

// ── سعر الذهب الحالي بالدولار لكل غرام (تقريبي — يُستبدل بـ API حقيقي) ──
const GOLD_PRICE_PER_GRAM_USD = 98.5;

// ── مكوّن لوحة المفاتيح الرقمية — يُستخدم في حقول الإدخال العددي بالحاسبة ──
function NumPad({ onPress, onDelete, onClear }: { onPress: (v: string) => void; onDelete: () => void; onClear: () => void }) {
  // تخطيط المفاتيح: 3 أعمدة بترتيب لوحة المفاتيح الكلاسيكية
