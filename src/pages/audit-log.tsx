...
import React, { useState } from "react";
  ...
  // ── إعدادات مستويات الخطورة: اللون والأيقونة لكل مستوى ──
  const severityConfig = {
    critical: { label: "Critical", cls: "bg-destructive/15 text-destructive border-destructive/30", icon: "🔴" }, // حرج
    warning:  { label: "Warning",  cls: "bg-yellow-500/15 text-yellow-400 border-yellow-500/30", icon: "🟡" },   // تحذير
    info:     { label: "Info",     cls: "bg-blue-500/15 text-blue-400 border-blue-500/30", icon: "🔵" },         // معلومة
  };

  // ── خريطة ألوان أنواع الكيانات في سجل التدقيق ──
  const entityConfig: Record<string, string> = {
    Order:    "bg-primary/15 text-primary",          // طلب
    Movement: "bg-green-500/15 text-green-400",      // حركة
    Box:      "bg-purple-500/15 text-purple-400",    // صندوق
    Tree:     "bg-emerald-500/15 text-emerald-400",  // شجرة
    Worker:   "bg-orange-500/15 text-orange-400",    // عامل
    QC:       "bg-cyan-500/15 text-cyan-400",        // جودة
  };

  export default function AuditLogPage() {
    const { t } = useTranslation();
    // ── استخراج سجلات التدقيق من الحالة العامة ──
    const { auditLogs } = useMockState();
    // ── نص البحث في سجلات التدقيق ──
    const [search, setSearch] = useState("");
    // ── فلتر مستوى الخطورة: all / critical / warning / info ──
    const [severityFilter, setSeverityFilter] = useState("all");
    // ── فلتر نوع الكيان: all / Order / Movement / Box / ... ──
    const [entityFilter, setEntityFilter] = useState("all");
    // ── السجل المحدد لعرض تفاصيله الكاملة ──
    const [selectedLog, setSelectedLog] = useState<AuditLog | null>(null);

    // ── تصفية السجلات بتطبيق جميع الفلاتر النشطة معاً ──
    const filtered = auditLogs.filter(log => {
      // البحث في عدة حقول معاً: الكود والمرجع والمحرر والحقل والسبب
