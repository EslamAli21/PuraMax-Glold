...
export default function SettingsPage() {
  // ── استخراج اسم المصنع من سياق المصادقة لعرضه في حقل الإعدادات ──
  const { factoryName } = useAuth();
  // ── استخراج حالة الثيم (فاتح/داكن) ودالة التغيير ──
  const { theme, setTheme } = useTheme();
  const { toast } = useToast();
  const { t } = useTranslation();

  // ── مرجع إلى حقل رفع الملف لتشغيله برمجياً عند الضغط على الزر ──
  const fileInputRef = useRef<HTMLInputElement>(null);
  // ── حالة ملف بيانات الاستيراد المرفوع ──
  const [uploadedFile, setUploadedFile] = useState<File | null>(null);
  // ── حالة عملية الرفع: idle → uploading → done ──
  const [uploadStatus, setUploadStatus] = useState<"idle" | "uploading" | "done">("idle");

  // ── حفظ الإعدادات: يعرض إشعاراً بنجاح الحفظ ──
  const handleSave = () => {
    toast({ title: t("settings.saved"), description: t("settings.savedDesc") });
  };

  // ── معالج تغيير ملف الاستيراد: يحاكي عملية رفع من 1.5 ثانية ──
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    setUploadedFile(file);
    setUploadStatus("uploading");
    // محاكاة وقت الرفع — يُستبدل بطلب API حقيقي في الإنتاج
