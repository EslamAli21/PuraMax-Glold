// ============================================================
// صفحة التنبيهات — عرض وإدارة جميع تنبيهات الإنتاج والتحذيرات
// ============================================================
...
export default function AlertsPage() {
  // ── استخراج بيانات التنبيهات ودوال التحديث من الحالة العامة ──
  const { alerts, orders, sections, markAlertRead, dismissAlert } = useMockState();
  // ── فلتر التبويبات: all / unread / high / medium / low ──
  const [filter, setFilter] = useState("all");
  const { t } = useTranslation();

  // ── تصفية التنبيهات: استبعاد المُرفَضة أولاً ثم تطبيق فلتر التبويب ──
  const filteredAlerts = alerts.filter(a => {
    if (a.isDismissed) return false;                                          // تجاهل المُرفَضة
    if (filter === "unread") return !a.isRead;                                // غير المقروءة فقط
    if (filter === "high" || filter === "medium" || filter === "low") return a.severity === filter; // حسب الأولوية
