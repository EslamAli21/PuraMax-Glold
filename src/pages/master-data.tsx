...
// ── نوع التبويب النشط — محدود بقائمة أقسام البيانات الرئيسية ──
type TabKey = "departments" | "machines" | "workers" | "employees" | "customers" | "materials" | "stamps";

export default function MasterDataPage() {
  // ── استخراج جميع بيانات النظام ودوال الإضافة من الحالة العامة ──
  ...
  // ── التبويب النشط — يحدد الجدول المعروض حالياً ──
  const [activeTab, setActiveTab] = useState<TabKey>("departments");
  // ── نص البحث للتصفية الفورية في الجداول ──
  const [search, setSearch] = useState("");
  // ── نوع حوار الإجراء الحالي: إضافة / تعديل / مغلق ──
  const [dialogType, setDialogType] = useState<"add" | "edit" | null>(null);
  // ── السجل المُراد تعديله — يُحفظ هنا عند فتح حوار التعديل ──
  const [editRecord, setEditRecord] = useState<any>(null);

  // ── بيانات النموذج المؤقتة للحوار (إضافة/تعديل) ──
  const [formData, setFormData] = useState<Record<string, any>>({});

  // ── فتح حوار الإضافة: يُصفّر النموذج ويُغلق التعديل ──
  const openAdd = () => {
    setFormData({});
    setEditRecord(null);
    setDialogType("add");
  ...
  // ── فتح حوار التعديل: يملأ النموذج ببيانات السجل الحالي ──
  ...
  // ── إغلاق الحوار وتصفير الحالة المؤقتة ──
  const closeDialog = () => {
    setDialogType(null);
    setEditRecord(null);
    setFormData({});
