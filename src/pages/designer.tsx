...
export default function DesignerPage() {
  // ── جلب الطلبات والعملاء من الحالة العامة للتطبيق ──
  const { orders, customers } = useMockState();
  // ── التبويب النشط: new / in-design / print ──
  const [activeTab, setActiveTab] = useState("new");
  const { t } = useTranslation();

  // ── تصفية الطلبات الجديدة المعلقة أو المعتمدة كموديلات جديدة ──
  const newAssignments = orders.filter(o => o.status === "pending" || (o.status === "approved" && o.isNewModel));
  // ── الطلبات قيد التصميم: في الإنتاج + موديل جديد ──
